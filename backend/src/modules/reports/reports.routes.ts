...
const payrollReportQuerySchema = z.object({
    ...
    res.status(200).json(
      ok({
        ...
        summary: {
          ...
          totalGross: payrolls.reduce((total, payroll) => total + payroll.totalGross, 0),
          totalDeductions: payrolls.reduce(
            (total, payroll) => total + payroll.totalDeductions,
            0
          ),
          totalNet: payrolls.reduce((total, payroll) => total + payroll.totalNet, 0)
        }
      })
    );
  })
