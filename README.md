...
- `hrms-postgres`: PostgreSQL database
...
To seed demo data in the hosted database, run this command with Render's backend environment loaded:

```bash
npm run db:seed --workspace backend
```

For local seeding against Render PostgreSQL, use the database's external connection string from the Render dashboard as `DATABASE_URL`, then run the same seed command.
